...
public class UserServiceImpl implements UserService {
	...
	public JsonNode saveUserDetailsInDB(String email, String password) {
		...
		try {
			...
			UserRole userRole = userDto.getRoles().stream()
		            .findFirst()
		            .orElseThrow(() -> new RuntimeException("UserRole not found"));
			
			logger.info("UserRole " + userRole.toString());
	...
	@Override
	public JsonNode updatePhone(Long userId, Long newPhone) {
		...
		try {
			...
			if(optionalUser.isPresent()) {
				...
				userRepository.save(user);
				dataObject.put(AppConstants.STATUS_CODE, ResponseConstants.PHONE_CHANGED);
			}
			else {
				errorObject.put(AppConstants.ERROR_CODE, ResponseConstants.INVALID_USER);
				errorObject.put(AppConstants.ERROR_REASON, "User Not Found");
